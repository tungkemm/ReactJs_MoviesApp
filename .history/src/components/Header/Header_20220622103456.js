...
const Header = () => {
  ...
  const handleSearchHistory = (data, id) => {
    dispatch(fetchAsyncMovies(data[id]));
    dispatch(fetchAsyncShows(data[id]));
  }
            ...
            <div className="search-history">
              ...
              <ul>
                {searchHistory.map((data, index) => 
                  <li key={index} onClick={() => handleSearchHistory(data, index)}>{data}</li>
                )}
              </ul>
            </div>
