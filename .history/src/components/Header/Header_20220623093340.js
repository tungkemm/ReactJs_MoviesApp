...
import { deleteItemSearchHistory, listSearchHistory } from "../../features/movies/filterSearchSlice";
import {
...
const Header = () => {
  ...
  const handleDeleteSearchHistory = (idSearchHistory) => {
    console.log(123)
              ...
              <ul>
                {searchHistory.map((data, index) => (
                  <li key={index} onClick={() => handleSearchHistory(data.item)}>
                    <span>{data.item}</span>
                    <i onClick={() => handleDeleteSearchHistory(data.id)} class="fa-solid fa-xmark"></i>
                  </li>
                ))}
              </ul>
            </div>
